...
void lmm_free(lmm_t *lmm, void *block, int size)
{
	...
	size = (((unsigned int)block & ALIGN_MASK) + size + ALIGN_MASK)
		& ~ALIGN_MASK;
	...
	for (reg = lmm->regions; ; reg = reg->next)
	{
		...
		if (((unsigned int)node >= reg->min)
		    && ((unsigned int)node < reg->max))
			break;
	}
	...
	if ((prevnode) &&
	    ((unsigned int)prevnode + prevnode->size >= (unsigned int)node))
	{
		assert((unsigned int)prevnode + prevnode->size
		       == (unsigned int)node);

		/* Coalesce prevnode with nextnode if possible.  */
		if (((unsigned int)nextnode)
		    && ((unsigned int)node + size >= (unsigned int)nextnode))
		{
			assert((unsigned int)node + size
